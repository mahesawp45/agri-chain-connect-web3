    ...
    <MainLayout>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="pricing" className="space-y-4">
              ...
              <div className="space-y-6">
                ...
                <Form {...priceForm}>
                  <form onSubmit={priceForm.handleSubmit(onPriceSubmit)} className="space-y-6">
                    ...
                    {transaction.price && (
                      <div className="p-4 rounded-lg bg-earth-wheat/40">
                        <h3 className="font-medium text-earth-dark-green mb-2">Current Price</h3>
                        <div className="flex justify-between items-center">
                          <span className="text-earth-medium-green">Price per {transaction.unit}:</span>
                          <span className="font-bold text-earth-dark-green">{formatCurrency(transaction.price)}</span>
                        </div>
                        <div className="flex justify-between items-center mt-2">
                          <span className="text-earth-medium-green">Total Amount:</span>
                          <span className="font-bold text-earth-dark-green">{formatCurrency(transaction.totalPrice)}</span>
                        </div>
                      </div>
                    )}
          ...
          <Card className="earth-card-clay overflow-hidden">
            <CardHeader className="earth-header-clay pb-3">
              <CardTitle className="text-white">Negotiation Guidelines</CardTitle>
            </CardHeader>
            <CardContent className="mt-4">
              <div className="space-y-4">
                <div className="p-4 rounded-lg bg-earth-clay/20">
                  <h3 className="font-medium text-earth-dark-green mb-2">Tips for Successful Negotiation</h3>
                  <ul className="list-disc pl-5 space-y-2 text-earth-medium-green">
                    <li>Consider current market prices when setting your price</li>
                    <li>Be transparent about quality and supply timing</li>
                    <li>Use WhatsApp to discuss details with the buyer</li>
                    <li>Be open to reasonable counteroffers</li>
                    <li>Document all agreements in the system once finalized</li>
                  </ul>
                </div>
                
                <div className="p-4 rounded-lg bg-earth-light-brown/20">
                  <h3 className="font-medium text-earth-dark-green mb-2">Market Price Reference</h3>
                  <p className="text-earth-medium-green mb-3">Current market prices for {transaction.commodityName}:</p>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center p-2 rounded bg-earth-pale-green/40">
                      <span className="text-earth-medium-green">Premium Quality</span>
                      <span className="font-medium text-earth-dark-green">Rp 12,000 - Rp 15,000 / kg</span>
                    </div>
                    <div className="flex justify-between items-center p-2 rounded bg-earth-pale-green/40">
                      <span className="text-earth-medium-green">Standard Quality</span>
                      <span className="font-medium text-earth-dark-green">Rp 9,000 - Rp 12,000 / kg</span>
                    </div>
                    <div className="flex justify-between items-center p-2 rounded bg-earth-pale-green/40">
                      <span className="text-earth-medium-green">Economy Quality</span>
                      <span className="font-medium text-earth-dark-green">Rp 7,000 - Rp 9,000 / kg</span>
                    </div>
                  </div>
                  <p className="text-xs text-earth-medium-green mt-2">
                    *Prices are for reference only and may vary based on region and other factors
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="mt-4">
              <div className="space-y-6">
                {transaction.termsDocUrl ? (
                  <>
                    <div className="p-4 rounded-lg bg-earth-pale-green/50">
                      <h3 className="text-md font-medium text-earth-dark-green mb-3">Terms Document</h3>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <FileText className="h-5 w-5 text-earth-medium-green" />
                          <span className="text-earth-dark-green">Transaction_Terms.pdf</span>
                        </div>
                    ...
                    <div className="p-4 rounded-lg bg-earth-light-green/20">
                      <h3 className="text-md font-medium text-earth-dark-green mb-3">
                        {transaction.signatureUrl ? 'Document Signed' : 'Signature Required'}
                      </h3>
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            <div className="flex items-center gap-2 mb-2">
                              <CheckCircle className="h-5 w-5 text-earth-medium-green" />
                              <span className="text-earth-dark-green font-medium">Terms approved and signed</span>
                            </div>
                            <p className="text-sm text-earth-medium-green">
                              Signed on: {formatDate(new Date(transaction.updatedAt))}
                            </p>
                          </div>
                          <img 
                            src={transaction.signatureUrl} 
                            alt="Signature" 
                            className="h-16 w-32 object-contain border border-earth-light-brown/30 rounded bg-white p-1" 
                          />
                        </div>
                      ) : (
                        <div className="space-y-4">
                          <p className="text-earth-medium-green">
                            Please upload your signature to approve the terms and conditions.
                          </p>
                          ...
                          <div className="flex items-center gap-4">
                            <Input
                              ...
                              className="hidden"
                            />
                            <div className="flex-1 border-dashed border-2 border-earth-light-brown/50 rounded-lg p-4 flex flex-col items-center justify-center">
                              {signature ? (
                                <div className="text-center">
                                  <CheckCircle className="h-8 w-8 text-earth-medium-green mx-auto mb-2" />
                                  <p className="text-earth-dark-green font-medium">{signature.name}</p>
                                  <p className="text-xs text-earth-medium-green mt-1">
                                    File size: {(signature.size / 1024).toFixed(2)} KB
                                  </p>
                                </div>
                              ) : (
                                <div className="text-center">
                                  <Upload className="h-8 w-8 text-earth-medium-green mx-auto mb-2" />
                                  <p className="text-earth-dark-green font-medium">Upload Signature</p>
                                  <p className="text-xs text-earth-medium-green mt-1">
                            ...
                            <label 
                              ...
                              className={`inline-flex items-center justify-center gap-2 p-2 bg-earth-medium-green text-white rounded-lg cursor-pointer ${signature ? 'hover:bg-earth-dark-green' : 'hover:bg-earth-medium-green/90'}`}
                            >
                              {signature ? <Pen className="h-4 w-4" /> : <Upload className="h-4 w-4" />}
                  ...
                  <div className="text-center py-8 bg-earth-pale-green/30 rounded-lg">
                    <FileText className="h-12 w-12 mx-auto mb-2 text-earth-light-green" />
                    <p className="mb-4 text-earth-medium-green font-medium">Terms document not available yet</p>
                    <p className="text-sm text-earth-medium-green">
                      The terms and conditions document will be available once the buyer submits it after price negotiation.
                    </p>
                  </div>
                )}
                ...
                <div className="p-4 rounded-lg bg-earth-wheat/30">
                  <h3 className="text-md font-medium text-earth-dark-green mb-3">Standard Terms</h3>
                  <div className="space-y-3 text-earth-medium-green">
                    <p>1. <span className="font-medium text-earth-dark-green">Quality:</span> The commodity must match the quality described in the listing.</p>
                    <p>2. <span className="font-medium text-earth-dark-green">Delivery:</span> Seller is responsible for arranging delivery to the buyer's specified location.</p>
                    <p>3. <span className="font-medium text-earth-dark-green">Payment:</span> Payment will be processed through the platform's escrow system.</p>
                    <p>4. <span className="font-medium text-earth-dark-green">Cancellation:</span> Cancellation terms will be specified in the detailed agreement.</p>
                    <p>5. <span className="font-medium text-earth-dark-green">Dispute Resolution:</span> All disputes will be handled according to the platform's dispute resolution process.</p>
                  </div>
                </div>
              </div>
            </CardContent>
