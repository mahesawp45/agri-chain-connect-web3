...
const TransactionDetail = () => {
  ...
  // Handle confirm transaction function
  const handleConfirmTransaction = () => {
    setTransaction(prev => {
      ...
      return {
        ...prev,
        status: "dikonfirmasi" as TransactionStatus,
        updatedAt: new Date(),
        history: [
          ...(prev.history || []),
          {
    ...
    toast({
      ...
      description: language === "id" 
        ? "Anda telah mengonfirmasi transaksi ini" 
        : "You have confirmed this transaction",
    });
  };

  // Handle decline transaction function
  const handleDeclineTransaction = () => {
    setTransaction(prev => {
      ...
      return {
        ...prev,
        status: "dibatalkan" as TransactionStatus,
        updatedAt: new Date(),
        history: [
          ...(prev.history || []),
          {
    ...
    toast({
      ...
      description: language === "id" 
        ? "Anda telah menolak transaksi ini" 
        : "You have declined this transaction",
      variant: "destructive",
    });
  };
  
  // Handle proceed to negotiation function
  const handleProceedToNegotiation = () => {
    navigate(`/transaksi/${transaction.id}/harga`);
  };

  // Handle print receipt function
  const handlePrintReceipt = () => {
    toast({
      title: language === "id" ? "Cetak Bukti Transaksi" : "Print Transaction Receipt",
      description: language === "id" 
        ? "Menyiapkan dokumen untuk dicetak..." 
        : "Preparing document for printing...",
    });
    
    // In a real application, this would trigger a print dialog or download a PDF
    setTimeout(() => {
      window.print();
    }, 500);
  };
    ...
    <MainLayout>
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-earth-dark-green">
            {language === "id" ? "Detail Transaksi" : "Transaction Detail"}
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          {/* For completed transactions, show the enhanced completion card */}
          {isCompleted ? (
            <TransactionComplete 
          ...
          {/* Status Cards - Only show for relevant statuses */}
          ...
          {/* Show shipping preparation */}
          {transaction?.status === "persiapan_pengiriman" && (
            <Card className="earth-card-clay overflow-hidden">
              <CardHeader className="earth-header-clay pb-3">
                <CardTitle className="text-white">
                  {language === "id" ? "Persiapan Pengiriman" : "Shipping Preparation"}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 mt-2">
                <div className="p-4 bg-earth-clay/20 rounded-lg">
                  <div className="flex items-center space-x-2 mb-3">
                    ...
                    <h3 className="font-medium text-earth-dark-green">
                      {language === "id" ? "Siapkan Komoditas untuk Dikirim" : "Prepare Commodities for Shipping"}
                    </h3>
                  </div>
                  
                  <p className="text-earth-medium-green mb-4">
                    {language === "id" 
                      ? "Silakan siapkan komoditas untuk pengiriman. Pastikan semua komoditas sudah diperiksa dan dikemas dengan aman."
                      : "Please prepare the commodities for shipping. Make sure all commodities have been checked and safely packed."}
                  </p>
